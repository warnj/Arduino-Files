...
int x = 0;
int xMin = 1053; // should be INTEGER.MAX_VALUE
int xMax = 1875; // should be INTEGER.MIN_VALUE
...
//   pinMode(6, OUTPUT);
   ...
   digitalWrite(13, HIGH); // begin calibration for 5 sec
   while (millis() < 5000) {
      x = pulseIn(XPIN, HIGH);
      y = pulseIn(YPIN, HIGH);
   ...
   digitalWrite(13, LOW); // end calibration
...
void loop() {
   x = map(pulseIn(XPIN, HIGH), xMin, xMax, -255, 255);
   y = map(pulseIn(YPIN, HIGH), yMin, yMax, 0, 255);
...
//   Serial.println(y);
   ...
   dir = pulseIn(DIRPIN, HIGH);
//   Serial.println(dir);
   dir = (dir < dirThresh) ? FORWARD : BACKWARD;
   motor(1, dir, y);

    xdir = (x < 0) ? BACKWARD : FORWARDg;
    motor(2, xdir, abs(x));
}
